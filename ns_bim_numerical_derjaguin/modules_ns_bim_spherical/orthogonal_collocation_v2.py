...
def get_nu(coeff_vec, t):
    total = 0.0
    for i in range(len(coeff_vec) - 2):
...
def get_yp_d1(coeff_vec, t, kap_h):
    c1 = coeff_vec[-1]
    c2 = coeff_vec[-2]
    b1 = get_b1(c1, kap_h, t)
    b2 = get_b2(c2, kap_h, t)
...
def get_yp_d2(coeff_vec, t, kap_h):
    c1 = coeff_vec[-1]
    c2 = coeff_vec[-2]
    b3 = get_b3(kap_h, t)
    nu = get_nu(coeff_vec, t)
...
def get_psi_from_phi(phi):
    ...
    kT = hmoc.constants().kT
    e0 = hmoc.constants().e0
    return e0*phi/kT
...
def unscale_energy(f_plus, area, bulk_solution):
    return f_plus*area*bulk_solution.ion_dens*hmoc.constants().kT/bulk_solution.kappa
...
class cp_bc_info:
    ...
    def get_sigma_02_star(self, coeff_vec):
        t     = 1.0
        yp    = get_yp(coeff_vec, t, self.kap_h)
        yp_d1 = get_yp_d1(coeff_vec, t, self.kap_h)
...
def get_c1_initial_guess(bc_info, bulk_solution):
    phi = hmoc.inverse_Grahame(bc_info.sigma_01, bulk_solution)
    psi = get_psi_from_phi(phi)
    y   = get_y_from_psi(psi)
    return y

def scaled_1d_pb_residual(coeff_vec, t, bc_info):
    yp    = get_yp(coeff_vec, t, bc_info.kap_h)
    yp_d1 = get_yp_d1(coeff_vec, t, bc_info.kap_h)
    yp_d2 = get_yp_d2(coeff_vec, t, bc_info.kap_h)
    if -1.0 < yp < 1.0:
...
def get_residual_vector(coeff_vec, bc_info):
    ...
    residual_vec[-2] = coeff_vec[-2] - bc_info.c2
    return residual_vec
...
def get_ref_energy(sigma_01, phi_02, bulk_solution):
    h         = 10.0/bulk_solution.kappa  # 10 DL separation
...
# # Need to update, now that c1 and c2 are placed at the end of coeff_vec
...
def get_residual_vector_cc(coeff_vec, bc_info, t_vec):
    residual_vec = np.zeros(len(coeff_vec))
    residual_vec = scaled_1d_pb_residual_cc(coeff_vec, t_vec, bc_info)
    residual_vec[-1], residual_vec[-2] = scaled_bc_residual_cc(coeff_vec, bc_info)
    return residual_vec

def get_coeff_vec_cc(guess, bc_info):
    p     = len(guess) - 2
    t_vec = np.full(len(guess), 0.5)
    for i in range(p):
        t_vec[i] = 1.0/(2.0*(p)) + i/(p)
...
def get_c1_c2_initial_guess(bc_info, bulk_solution):
    phi_1 = hmoc.inverse_Grahame(bc_info.sigma_01, bulk_solution) # NLPBE
    psi_1 = get_psi_from_phi(phi_1)
    y_1   = get_y_from_psi(psi_1)

    phi_2 = hmoc.inverse_Grahame(bc_info.sigma_02, bulk_solution) # NLPBE
...
def get_ref_energy_v3(sigma_01, sigma_02, bulk_solution):
    ...
    guess[-1], guess[-2] = get_c1_c2_initial_guess(bc_info, bulk_solution)
...
def get_energy_for_seps_v3(args):
    ...
    (t1_ref, t2_ref, t3_ref, t4_ref) = get_ref_energy_v3(sigma_01, sigma_02, bulk_solution)
    for i in range(len(energy_t1)):
        energy_t1[i] -= t1_ref
        energy_t2[i] -= t2_ref
        energy_t3[i] -= t3_ref
    ...
    results = []
    for i in range(len(seps)):
        results.append((seps[i], sigma_02, energy_t1[i], energy_t2[i],
                        energy_t3[i], energy_t4[i]))
    return results
