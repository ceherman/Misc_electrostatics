...
MODULE DH_SurfCal_FreeEnergy
    ...
    SUBROUTINE GetPhidPhidnInf
      ...
      !    DO igroup = 1, ttlgroup
              ...
              xloctn(1) = -1.0*ref_farfield
              yloctn(1) = ref_farfield
              zloctn(1) = 0.0d0
              xloctn(2) = ref_farfield
              yloctn(2) = ref_farfield
              zloctn(2) = ref_farfield
              xloctn(3) = -1.0*ref_farfield
              yloctn(3) = -1.0*ref_farfield
              zloctn(3) = -1.0*ref_farfield
              xloctn(4) = -1.0*ref_farfield
              yloctn(4) = -1.0*ref_farfield
              zloctn(4) = ref_farfield
            ...
            ! Record potential (mV) at each node for each radius
              OPEN (857, FILE = "rslt_nodes_coords_and_pots.csv", STATUS="REPLACE", ACTION="WRITE")
                  WRITE(857,*) 'node_id,x_nm,y_nm,z_nm,phi_mV,charge_C_m2'
                  DO i = 1, ttlnmbrnd
                      CALL csv_write_integer(857,i,.false.,'cmr')
                      CALL csv_write_dble(857,xnd(i),.false.,'cmr')
                      CALL csv_write_dble(857,ynd(i),.false.,'cmr')
                      CALL csv_write_dble(857,znd(i),.false.,'cmr')
                      CALL csv_write_dble(857,exPhi3_DH(i)*phi_dim,.false.,'cmr')
                      CALL csv_write_dble(857,exPhi3dn_DH(i)*exeps_DH*Sigma_dim,.true.,'spc')
                  END DO
              CLOSE (857)
